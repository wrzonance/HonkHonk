...
> Follows the global `~/.claude/rules` (code · workflow · security · agents). This file holds only
> HonkHonk-specific facts and overrides. Full design spec: `ARCHITECTURE.md`. Decisions: `docs/adr/`.

## Project overrides (tighten / differ from the global rules)

- **File size: 400 lines max** (global default is 800) — split before adding. Functions ≤50 lines.
- **Coverage: 80% target** via `cargo tarpaulin` every CI run (diagnostic globally; a target here).
- **Known violation:** `src/app.rs` is ~1,655 lines. Do NOT add to it — split first.

## Architecture (summary — full spec in ARCHITECTURE.md)
...
justifying why stdlib/existing deps can't do it; a new system `-dev` dep must update
`.github/workflows/*.yml` in the same PR so CI doesn't break.

## Multi-DE rules (mandatory)
...
- Shortcuts `org.freedesktop.portal.GlobalShortcuts` (ashpd, Phase 2) · File dialogs
  `…portal.FileChooser` · Notifications `…portal.Notification` · Autostart `…portal.Background`.
...
- Runtime deps NOT bundled (except AppImage/Flatpak) — packages declare deps. `packaging/` holds
...
play-sound-to-virtual-mic integration path. Don't test Iced view rendering or third-party internals.
